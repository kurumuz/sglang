...
class TokenizerCommunicatorMixin:
    ...
    def init_communicators(self: TokenizerManager, server_args: ServerArgs):
        ...
        self.check_cache_communicator = _Communicator(
            self.send_to_scheduler, server_args.dp_size
        )
    ...
    def _get_communicator_dispatcher(self: TokenizerManager):
        return TypeBasedDispatcher(
            [
                ...
                (
                    CheckCacheReqOutput,
                    self.check_cache_communicator.handle_recv,
                ),
                (
                    ProfileReqOutput,
    ...
    async def check_cache(self: TokenizerManager, input_ids: List[int], extra_key: Optional[str] = None) -> CheckCacheReqOutput:
        """Check if a cache entry exists for the given token sequence."""
        return (await self.check_cache_communicator(CheckCacheReqInput(input_ids=input_ids, extra_key=extra_key)))[0]
